...
class BreweryAdapter (private var brewery: ArrayList<BrewingInformation>, private var context: Context, private var breweryAdapterCallback: BreweryAdapterInterface): RecyclerView.Adapter<BreweryAdapter.BreweryAdapterViewHolder>(){
    interface BreweryAdapterInterface{
        fun deleteFav(id: String)
        fun addtoFav (name: String, brewType: String, country: String,city:String, state: String)
    }
    inner class BreweryAdapterViewHolder(private var binding: BreweriesContentRvBinding): RecyclerView.ViewHolder(binding.root){
            ...
            with(binding) {
                ...
                binding.favbtn.setOnClickListener{
                    if(binding.favbtn.tag.equals("favorite")){
                        binding.favbtn.setImageResource(R.drawable.favorited)
                        binding.favbtn.tag = "favorited"
                        breweryAdapterCallback.addtoFav(itemData.name, itemData.brewery_type, itemData.country,itemData.city, itemData.state)
                    }
                    else{
                        binding.favbtn.setImageResource(R.drawable.favorite)
                        binding.favbtn.tag = "favorite"
                        breweryAdapterCallback.deleteFav(itemData.id)
                    }
                }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BreweryAdapterViewHolder {
        val binding = BreweriesContentRvBinding.inflate(LayoutInflater.from(parent.context),parent, false)
        return BreweryAdapterViewHolder(binding)
    }

    override fun onBindViewHolder(holder: BreweryAdapterViewHolder, position: Int) {
        val brewery = brewery[position]
        return holder.bind(brewery)
    }
    ...
    fun updateData(breweryList: ArrayList<BrewingInformation>){
        this.brewery.addAll(breweryList)
        this.notifyItemInserted(brewery.size)
    }

    fun resetData(){
        this.brewery.clear()
        this.brewery = arrayListOf()
